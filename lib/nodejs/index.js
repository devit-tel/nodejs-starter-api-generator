...
const main = async () => {
    ...
    const { packageName, destinationPath } = await inquirer.prompt(questionsSectionTwo)
    ...
    if (gitSource === 'Private' && tagVersion === 'v1.0.0') {
    ...
    let fileContentOfPackage = (fs.readFileSync(path.join(destinationPath, 'package.json'), 'utf8')).toString()
    fileContentOfPackage = fileContentOfPackage.replaceAll('<%package-name%>', packageName)
    console.info(`Replaced <%package-name%> with ${packageName} inside package.json`)

    let fileContentOfgitlab = (fs.readFileSync(path.join(destinationPath, '.gitlab-ci.yml'), 'utf8')).toString()

    fileContentOfgitlab = fileContentOfgitlab.replaceAll('<$registry-name$>', deploymentVar.registryName)
    console.info(`Replaced <$registry-name$> with ${deploymentVar.registryName} inside .gitlab-ci.yml`)
    fileContentOfgitlab = fileContentOfgitlab.replaceAll('<$project-repo-name$>', deploymentVar.projectRepoName)
    console.info(`Replaced <$project-repo-name$> with ${deploymentVar.projectRepoName} indside .gitlab-ci.yml`)
    fileContentOfgitlab = fileContentOfgitlab.replaceAll('<$helm-production-name$>', deploymentVar.helmProductionName)
    console.info(`Replaced <$helm-production-name$> with ${deploymentVar.helmProductionName} inside .gitlab-ci.yml`)
    
    let fileContentOfvalueProduction = fs.readFileSync(path.join(destinationPath, 'deployment', 'values-production-th.yaml'), 'utf8')
    
    fileContentOfvalueProduction = fileContentOfvalueProduction.replaceAll('<$name-override$>', deploymentVar.nameOverride)
    console.info(`Replaced <$name-override$> with ${deploymentVar.nameOverride} inside inside deployment/values-production-th.yaml`)
    fileContentOfvalueProduction = fileContentOfvalueProduction.replaceAll('<$registry-name$>', deploymentVar.registryName)
    console.info(`Replaced <$registry-name$> with ${deploymentVar.registryName} inside inside deployment/values-production-th.yaml`)
    fileContentOfvalueProduction = fileContentOfvalueProduction.replaceAll('<$default-port-http$>', deploymentVar.defaultPortHttpName)
    console.info(`Replaced <$default-port-http$> with ${deploymentVar.defaultPortHttpName} inside deployment/values-production-th.yaml`)
    
    console.info(`Overwriting .gitlab-ci.yml ...`)
    fs.writeFileSync(path.join(destinationPath, '.gitlab-ci.yml'), fileContentOfgitlab)
    console.info(`Overwriting .gitlab-ci.yml successful`)
    
    console.info(`Overwriting deployment/values-production-th.yaml ...`)
    fs.writeFileSync(path.join(destinationPath, 'deployment', 'values-production-th.yaml'), fileContentOfvalueProduction)
    console.info(`Overwriting deployment/values-production-th.yaml successful`)
    
    console.info(`Overwriting package.json ...`)
    fs.writeFileSync(path.join(destinationPath, 'deployment', 'package.json'), fileContentOfPackage)
